...
def _build_hover_texts(relationships: list, rel_type: str, is_bidirectional: bool) -> List[Optional[str]]:
...
def _create_directional_arrows(
    ...
    Uses vectorized NumPy operations for efficient arrow position calculation,
    which is significantly faster than Python loops for large graphs.

    Args:
        graph: The asset relationship graph
        positions: NumPy array of node positions (N x 3)
        asset_ids: List of asset IDs

    Returns:
        List containing a single Scatter3d trace for directional arrows, or empty list
    """
    relationship_set = _build_relationship_set(graph, set(asset_ids))
    asset_ids_set = set(asset_ids)
    asset_id_index = _build_asset_id_index(asset_ids)

    # Collect unidirectional relationships
    unidirectional_rels = []
    ...
    if not unidirectional_rels:
        return []

    # Extract indices as NumPy arrays for vectorized operations
    source_indices = np.array([rel["source_idx"] for rel in unidirectional_rels])
    target_indices = np.array([rel["target_idx"] for rel in unidirectional_rels])

    # Vectorized arrow position calculation (70% along edge towards target)
    # This is significantly faster than Python loop for large arrays
    ...
    arrow_x = arrow_positions[:, 0].tolist()
    arrow_y = arrow_positions[:, 1].tolist()
    arrow_z = arrow_positions[:, 2].tolist()

    # Build hover texts
    arrow_hovers = [
        f"Direction: {rel['source_id']} → {rel['target_id']}<br>Type: {rel['rel_type']}"
        for rel in unidirectional_rels
    ]
