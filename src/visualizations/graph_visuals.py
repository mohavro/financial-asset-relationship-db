...
def _validate_visualization_data(
    ...
    if not isinstance(colors, (list, tuple)) or len(colors) != n:
        got_len = len(colors) if isinstance(colors, (list, tuple)) else "N/A"
        raise ValueError(
            f"Invalid graph data: colors must be a list/tuple of length {n}, got {type(colors).__name__} with length {got_len}"
        )
    if not all(isinstance(c, str) and c for c in colors):
...
def visualize_3d_graph(graph: AssetRelationshipGraph) -> go.Figure:
    ...
    fig.add_trace(
        _create_node_trace(positions, asset_ids, colors, hover_texts)
    )
...
def _collect_and_group_relationships(
    ...
    relationship_filters: TypingOptional[Dict[str, bool]] = None,
...
def _build_edge_coordinates_optimized(
...
) -> Tuple[List[TypingOptional[float]], List[TypingOptional[float]], List[TypingOptional[float]]]:
    ...
    edges_x: List[TypingOptional[float]] = [None] * (num_edges * 3)
    edges_y: List[TypingOptional[float]] = [None] * (num_edges * 3)
    edges_z: List[TypingOptional[float]] = [None] * (num_edges * 3)
...
def _build_hover_texts(relationships: List[dict], rel_type: str, is_bidirectional: bool) -> List[TypingOptional[str]]:
    ...
    hover_texts: List[TypingOptional[str]] = [None] * (num_rels * 3)
...
def visualize_3d_graph_with_filters(
    ...
    # Generate dynamic title and configure layout
    dynamic_title = (
        f"Financial Asset Relationship Network - Enhanced 3D Visualization\n"
        f"({len(asset_ids)} Assets, {visible_relationships} Relationships)"
    )
