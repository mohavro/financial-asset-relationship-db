...
def _validate_visualization_data(
    ...
    if not np.isfinite(positions).all():
        ...
        inf_count = np.isinf(positions).sum()
        raise ValueError(
            f"Invalid graph data: positions must contain finite values (no NaN or Inf). Found {nan_count} NaN values and {inf_count} Inf values")
...
def visualize_3d_graph(graph: AssetRelationshipGraph) -> go.Figure:
    ...
    # Note: _create_relationship_traces returns a list of traces which are added
    # in a single batch operation below for optimal performance
    ...
    # (Performance: batch add_traces() is more efficient than loop with add_trace())
    arrow_traces = _create_directional_arrows(graph, positions, asset_ids)
    if arrow_traces:
        fig.add_traces(arrow_traces)
