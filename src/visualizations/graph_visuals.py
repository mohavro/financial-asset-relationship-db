...
def visualize_3d_graph(graph: AssetRelationshipGraph) -> go.Figure:
    ...
    # Create index mapping once for reuse
    ...
    arrow_traces = _create_directional_arrows(graph, positions, asset_ids, asset_id_to_idx)
...
def _collect_and_group_relationships(
    ...
    """
    # Convert asset_ids to set for O(1) membership testing
    asset_ids_set = set(asset_ids)
    ...
    for source_id, rels in graph.relationships.items():
        ...
        for target_id, rel_type, strength in rels:
            ...
            # Skip the second occurrence of a bidirectional relationship
            if reverse_exists and not is_bidirectional and pair_key in bidirectional_pairs:
                continue
...
def _create_relationship_traces(
    ...
    positions: np.ndarray,
    asset_ids: List[str],
    asset_id_to_idx: Optional[Dict[str, int]] = None,
    relationship_filters: Optional[Dict[str, bool]] = None,
) -> List[go.Scatter3d]:
    ...
    Args:
        graph: The asset relationship graph
        positions: Node positions in 3D space
        asset_ids: List of asset IDs
        asset_id_to_idx: Optional mapping from asset ID to position index (created if not provided)
        relationship_filters: Optional dict to filter relationship types

    Returns:
        List of Plotly traces for relationships
    """
    # Create index mapping if not provided
    if asset_id_to_idx is None:
        asset_id_to_idx = {asset_id: idx for idx, asset_id in enumerate(asset_ids)}
...
def _create_directional_arrows(
    ...
    Returns:
        List of Plotly traces for directional arrows
    """
    # Create index mapping if not provided
    if asset_id_to_idx is None:
        asset_id_to_idx = {asset_id: idx for idx, asset_id in enumerate(asset_ids)}
    ...
    # Pre-build a set of all relationships for faster lookup
    relationship_set = set()
    for source_id, rels in graph.relationships.items():
        if source_id not in asset_ids_set:
            continue
    ...
    # Find unidirectional relationships using the relationship set
    for source_id, rels in graph.relationships.items():
        if source_id not in asset_ids_set:
            continue
...
def visualize_3d_graph_with_filters(
    ...
    relationship_traces = _create_relationship_traces(
        graph, positions, asset_ids, asset_id_to_idx, relationship_filters
    )
