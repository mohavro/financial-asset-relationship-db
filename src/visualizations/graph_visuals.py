...
def _validate_visualization_data(
    ...
    if not isinstance(colors, (list, tuple)) or len(colors) != n:
        raise ValueError(f"Invalid graph data: colors must be a list/tuple of length {n}, got type {type(colors).__name__} with length {len(colors) if isinstance(colors, (list, tuple)) else 'unknown'}")
    if not isinstance(hover_texts, (list, tuple)) or len(hover_texts) != n:
        raise ValueError(f"Invalid graph data: hover_texts must be a list/tuple of length {n}, got type {type(hover_texts).__name__} with length {len(hover_texts) if isinstance(hover_texts, (list, tuple)) else 'unknown'}")
