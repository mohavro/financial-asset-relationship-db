...
def _build_relationship_index_safe(graph: AssetRelationshipGraph, asset_ids: Iterable[str]) -> Dict[Tuple[str, str, str], float]:
    asset_ids_set = set(asset_ids)
    relationship_index: Dict[Tuple[str, str, str], float] = {}
    for source_id, rels in getattr(graph, 'relationships', {}).items():
...
def _collect_and_group_relationships(
    ...
    if relationship_filters is not None:
        ...
        if invalid_keys:
            raise ValueError(
                f"Invalid relationship filter keys: {invalid_keys}. Valid keys are: {VALID_RELATIONSHIP_TYPES}"
            )

    relationship_index = _build_relationship_index_safe(graph, asset_ids)
...
def visualize_3d_graph(graph: AssetRelationshipGraph) -> go.Figure:
    ...
    _configure_3d_layout(
        fig,
        "Financial Asset Relationship Network - Enhanced 3D Visualization",
        width=1200,
        height=800,
        gridcolor="rgba(200, 200, 200, 0.3)",
        bgcolor="rgba(248, 248, 248, 0.95)",
        legend_bgcolor="rgba(255, 255, 255, 0.8)",
        legend_bordercolor="rgba(0, 0, 0, 0.3)",
    )
...
def visualize_3d_graph_with_filters(
    ...
    if not hasattr(graph, "get_3d_visualization_data_enhanced"):
        raise ValueError(
            "Invalid graph data provided: graph must have get_3d_visualization_data_enhanced method"
        )
    ...
    for param_name, param_value in filter_params.items():
        if not isinstance(param_value, bool):
            raise TypeError(
                f"Invalid filter configuration: {param_name} must be a boolean, got {type(param_value).__name__}"
            )
    ...
    try:
        ...
        raise ValueError(
            f"Invalid graph data: get_3d_visualization_data_enhanced method failed: {exc}"
        ) from exc
    except Exception as exc:
        raise RuntimeError(
    ...
    try:
        ...
        raise RuntimeError(
            f"Unexpected error during relationship trace creation: {exc}"
        ) from exc
    ...
    try:
        ...
        raise RuntimeError(
            f"Failed to add relationship traces to figure: {exc}"
        ) from exc
    ...
    if toggle_arrows:
        try:
            ...
            raise RuntimeError(
                f"Unexpected error during arrow creation: {exc}"
            ) from exc
