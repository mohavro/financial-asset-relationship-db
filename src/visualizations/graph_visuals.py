...
def _build_relationship_index(
    ...
    """
    # Validate graph type and structure
    if not isinstance(graph, AssetRelationshipGraph):
        raise TypeError("Expected graph to be an instance of AssetRelationshipGraph")
    if not hasattr(graph, "relationships"):
        raise ValueError("Invalid graph data: graph must have a 'relationships' attribute")
    if not isinstance(graph.relationships, dict):
        raise ValueError("Invalid graph data: graph.relationships must be a dictionary")

    # Validate and convert asset_ids to set
    try:
        asset_ids_set = set(asset_ids)
    except TypeError as exc:
        raise ValueError("Invalid input: asset_ids must be an iterable") from exc
    if not all(isinstance(aid, str) for aid in asset_ids_set):
        raise ValueError("Invalid input: all asset_ids must be strings")
    ...
    for source_id, rels in graph.relationships.items():
        ...
        # Validate rels structure
        if not isinstance(rels, (list, tuple)):
            raise ValueError(f"Invalid relationship data: relationships for '{source_id}' must be a list or tuple, got {type(rels).__name__}")

        for rel in rels:
            # Validate relationship tuple structure
            if not isinstance(rel, (list, tuple)) or len(rel) != 3:
                raise ValueError(f"Invalid relationship data: each relationship must be a 3-element tuple (target_id, rel_type, strength), got {type(rel).__name__} with length {len(rel) if isinstance(rel, (list, tuple)) else 'N/A'} for source '{source_id}'")

            target_id, rel_type, strength = rel

            # Validate target_id type
            if not isinstance(target_id, str):
                raise ValueError(f"Invalid relationship data: target_id must be a string, got {type(target_id).__name__} for source '{source_id}'")

            # Validate rel_type type
            if not isinstance(rel_type, str):
                raise ValueError(f"Invalid relationship data: rel_type must be a string, got {type(rel_type).__name__} for relationship '{source_id}' -> '{target_id}'")

            # Validate and convert strength to float
            try:
                strength_float = float(strength)
            except (TypeError, ValueError) as exc:
                raise ValueError(f"Invalid relationship data: strength must be numeric, got {type(strength).__name__} with value '{strength}' for relationship '{source_id}' -> '{target_id}'") from exc
...
def visualize_3d_graph(graph: AssetRelationshipGraph) -> go.Figure:
    ...
    arrow_traces = _create_directional_arrows(graph, positions, asset_ids)
    # Use batch operation for better performance when adding multiple traces
