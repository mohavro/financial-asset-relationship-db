...
def visualize_2d_graph(
    ...
    if show_all_relationships or any(
        [
            show_same_sector,
            show_market_cap,
            show_correlation,
    ...
    fig.add_trace(
        go.Scatter(
            ...
            y=y_coords,
            mode="markers+text",
            marker=dict(size=node_sizes, color=colors, line=dict(width=2, color="black"), opacity=0.8),
            text=asset_ids,
    ...
    fig.update_layout(
        title=dict(text="📈 2D Asset Relationship Network", x=0.5, font=dict(size=20, color="#2C3E50")),
        showlegend=True,
        hovermode="closest",
